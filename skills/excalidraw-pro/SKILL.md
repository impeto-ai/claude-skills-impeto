---
name: excalidraw-pro
description: Draw professional diagrams on a live Excalidraw canvas via MCP. Use when asked to create architecture diagrams, flowcharts, system maps, comparison visuals, or any visual explanation. Covers setup, 10 visual techniques, design preferences, layout best practices, and the screenshot-iterate loop. Activates for "diagrama", "fluxograma", "arquitetura", "mermaid", "mapa", "excalidraw", "fluxo", "desenhar diagrama", "diagram", "flowchart", "draw architecture".
---
...
## MCP Tools Reference (full surface)

This skill emphasizes the most-used tools (`batch_create_elements`, `get_canvas_screenshot`, `update_element`, `snapshot_scene`, `read_diagram_guide`, `clear_canvas`, `describe_scene`, `query_elements`, `set_viewport`, `create_from_mermaid`, `export_*`, `unlock_elements`, `get_element`).

**Other tools available** (call directly when the situation fits):

| Tool | When to use |
|------|-------------|
| `create_element` | Add a single element to an existing scene (cheaper than batch for 1-2 items) |
| `delete_element` | Remove a specific element without `clear_canvas` |
| `align_elements` | Align a group of elements on the X or Y axis (replaces manual coordinate math) |
| `distribute_elements` | Even spacing between 3+ elements (replaces manual gap counting) |
| `duplicate_elements` | Replicate one or more shapes (when you need N similar cards) |
| `group_elements` / `ungroup_elements` | Treat several shapes as a single unit for moving/aligning |
| `lock_elements` | Lock finished sections so further refines don't move them by accident |
| `import_scene` | Load a pre-existing `.excalidraw` JSON onto the canvas |
| `get_resource` | Retrieve a registered MCP resource (use only if the MCP server documents one) |

**Tip:** prefer `align_elements` + `distribute_elements` over recomputing x/y by hand. They're under-used because the skill historically didn't list them.
